# Software for modeling {#introduction}
...
```{r introduction-modeling-process, echo = FALSE, out.width = '100%', fig.width=8, fig.height=3, fig.cap = "A schematic for the typical modeling process.", warning = FALSE}
if (knitr:::is_html_output()) {
  file.copy("premade/modeling-process.svg", "_book/premade/modeling-process.svg")
  knitr::include_graphics("premade/modeling-process.svg")
} else {
  file.copy("premade/modeling-process.pdf", "_book/premade/modeling-process.pdf")
  knitr::include_graphics("premade/modeling-process.pdf")
}
```

This iterative process is especially true for modeling. Figure \@ref(fig:introduction-modeling-process) is meant to emulate the typical path to determining an appropriate model. The general phases are:

 * Exploratory data analysis (EDA). Initially there is a back and forth between numerical analysis and visualization of the data (represented in Figure \@ref(fig:introduction-data-science-model)) where different discoveries lead to more questions and data analysis "side-quests" to gain more understanding. 
 
 * Feature engineering. Understanding results in the creation of specific model terms that make it easier to accurately model the observed data. This can include complex methodologies (e.g., PCA) or simpler features (using the ratio of two predictors). 
...
The book is separated into parts. The first introduces an example data set and introduce some of the fundamental packages: `r pkg(recipes)`, `r pkg(parsnip)`, `r pkg(workflows)`, `r pkg(yardstick)`, and others. 
