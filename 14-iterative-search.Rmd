...
To demonstrate, let's look at a toy example with a single parameter that has values between [0, 1] and the performance metric is $R^2$. The true function is shown below in red in Figure \@ref(fig:performance-profile), along with `r nrow(current_iter)` candidate values that have existing results.
...
For these data, the GP model fit is shown in Figure \@ref(fig:estimated-profile). The shaded region indicates the mean $\pm$ 1 standard error. The two vertical lines indicate two candidate points that are examined in more detail below.
...
x_lab <- expression(Predicted ~ R^2 ~ Distribution)
...
Figure \@ref(fig:bo-surfaces) shows the surfaces of the mean, variance, and expected improvement surfaces estimated by the GP after 11 iterations. The right-hand panel shows a ridge of best estimated improvement along the right side of the candidate space.
...
```{r bo-search, echo = FALSE, warning = FALSE, dev = "png", out.width="100%", fig.width=9, fig.height=4}
#| echo = FALSE, 
#| message = FALSE, 
#| warning = FALSE,
#| dev = "png",
...
The first five iterations initially move in a poor direction but quickly move closer to better results.  The middle panel shows the process investigating the region of true optimal results with a short foray to the bottom right boundary of the candidate space. The remaining iterations switch between the region of best results and the far borders of the search space. 
...
How are the acceptance probabilities influenced? This heatmap in Figure \@ref(fig:acceptance-prob) shows how the acceptance probability can change over iterations, performance, and the user-specified coefficient.
...
```{r save, echo = FALSE}
save(svm_sa, file = "RData/svm_sa.RData", compress = TRUE, version = 2)
```