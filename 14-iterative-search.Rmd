...
Another virtue of this model is that, since a full probability model is specified, the predictions for new inputs can reflect the distribution of the outcome. In other words, the performance values can be predicted in terms of both mean and variance. 
...
As previously mentioned, Bayesian optimization is an iterative process. Based on the existing results, the GP model is fit, new candidates are predicted, and one is selected. We compute performance estimates for the new configuration, the GP is refit with the additional data point (and so on). 
