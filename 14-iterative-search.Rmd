...
To demonstrate, let's look at a toy example with a single parameter that has values between [0, 1] and the performance metric is $R^2$. The true function is shown below in red along with `r nrow(current_iter)` candidate values that have existing results in Figure \@ref(fig:performance-profile).

```{r performance-profile}
#| echo = FALSE, 
#| fig.height = 4,
#| fig.cap = "A hypothetical true performance profile over an arbitrary tuning parameter. Five estimated points are also shown.",
...
For these data, the GP model model fit is shown in Figure \@ref(fig:estimated-profile). The shaded region indicates the mean $\pm$ 1 standard error. The two vertical lines indicate two candidate points that are examined in more detail below.
...
#| echo = FALSE, 
#| fig.height = 4,
#| fig.cap = "The estimated performance profile generated by the Gaussian process model. The shaded region shows one-standard error bounds.",
...
For example, consider two candidate parameter values of 0.10 and 0.25 (indicated by the vertical lines in the plot above). Using the fitted GP model, their predicted $R^2$ distributions are shown in Figure \@ref(fig:two-candidates) along with a reference line for the current best results:
...
#| fig.alt = "Predicted performance distributions for two sampled tuning parameter values. For one, the distribution is slightly better than the current value with a small spread. The other parameter value os slightly worse but has a very wide spread."
small_pred <- 
...
When only considering the mean $R^2$ prediction, a parameter value of 0.10 is the better choice (see Table \@ref(tab:two-exp-improve)). The tuning parameter recommendation for 0.25 is, on average, predicted to be _worse_ than the current best. However, since it has higher variance, it has more overall probability area above the current best. As a result, it has a larger expected improvement of the two:
...
small_pred %>% 
  ...
  format(digits = 4, scientific = FALSE) %>% 
  kable(
    caption = "The expected improvement for the two candidate tuning parameters.",
...
The `autoplot()` function has several options for iterative search methods. Figure \@reg(fig:progress-plot) shows how the outcome changed over the search by using `autoplot(svm_bo, type = "performance")`. 

```{r progress-plot}
#| fig.height = 4,
#| echo = FALSE,
#| fig.cap = "The progress of the Bayesian optimization produced shown when the `autoplot()` method is used with `type = 'performance'`.",
#| fig.alt = "The progress of the Bayesian optimization produced shown when the `autoplot()` method is used with `type = 'performance'`. The plot shows the estimated performance on the y axis versus the iteration number on the x axis. Confidence intervals are shown for the points."
...
```{r iterative-bo-progress, eval = FALSE}
...
#| fig.height = 4.5,
#| out.width = "80%",
#| fig.cap = "A heatmap of the simulated annealing acceptance probabilities for different coefficient values.",
...
```{r iterative-neighborhood, echo = FALSE, }
#| echo = FALSE, 
#| message = FALSE, 
#| warning = FALSE,
#| out.width = "80%",
...
#| fig.height = 4,
#| echo = FALSE,
#| fig.cap = "The progress of the simulated annealing process shown when the `autoplot()` method is used with `type = 'performance'`.",
...
#| fig.height = 4,
#| echo = FALSE,
#| fig.cap = "A visualization of performance versus tuning parameter values when the `autoplot()` method is used with `type = 'parameters'`.",
...
```{r iterative-sa-plot, eval = FALSE}
av_capture_graphics(
