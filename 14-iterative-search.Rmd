...
large_grid <- 
  svm_large %>% 
  collect_metrics() %>% 
  distinct(cost, rbf_sigma)


x_rng <- 10^extendrange(log10(large_grid$rbf_sigma))
y_rng <- 2^extendrange(log2(large_grid$cost))
...
The figure below shows the surfaces of the mean, variance, and expected improvement surfaces estimated by the GP after 11 iterations. The right-hand panel shows a ridge of best estimated improvement along the right side of the candidate space.

```{r bo-surfaces, echo = FALSE, dev = "pdf", out.width="100%", fig.width=9, fig.height=4}
surf_mean + surf_sd + surf_impr
```

This figure shows the search process at three different points in the optimization:

```{r bo-search, echo = FALSE, warning = FALSE, dev = "pdf", out.width="100%", fig.width=9, fig.height=4}
...
```{r iterative-acceptance, echo = FALSE, fig.height = 4.5}
...
A visualization of the search path helps to understand where the search process did well and where it went astray. The plot below illustrated several "phases" of the optimization; these are separated by a restart of the process at the last best results. 

```{r iterative-sa-plot, echo = FALSE, warning = FALSE, dev = "pdf", out.width="100%", fig.width=10, fig.height=3.5}
...
sa_1 <- sa_path(1) + ggtitle("Phase 1")
sa_2 <- sa_path(2, FALSE) + ggtitle("Phase 2")
sa_3 <- sa_path(3, FALSE) + ggtitle("Phase 3")
sa_4 <- sa_path(4, FALSE) + ggtitle("Phase 4")
sa_5 <- sa_path(5, FALSE) + ggtitle("Phase 5")
sa_1 + sa_2 + sa_3 + sa_4 + sa_5 +  plot_layout(ncol = 5)
```
