...
As previously mentioned, grid search takes a pre-defined set of candidate values, evaluates them, then chooses the best. Iterative search methods take a different strategy. During the search process, they predict novel candidate values to test next. When grid search is infeasible or inefficient, iterative methods can be effective methods for optimizing tuning parameters. 

The chapter outlines two search methods. First, Bayesian optimization, which uses a statistical model to predict better settings, is discussed. After that, the chapter describes a global search method called simulated annealing. 

We use the same data for illustration but change the model. This chapter uses support vector machine model for illustration since it provides nice two-dimensional visualizations of the search processes.  
...
For illustration, the kernel parameter range is slightly changed (to improve the visualizations of the search): 
...
The main questions in this tuning scheme are how to create the model and how to select parameters predicted by that model. First, let's consider the technique most commonly used for Bayesian optimization, the Gaussian process model. 

### A Gaussian process model

Gaussian process (GP) [@SCHULZ20181] models are well-known statistical models that have a history in spatial statistics (under the name of _kriging methods_). They can be derived in multiple ways, including as a Bayesian model (see @RaWi06 for an excellent reference). 

Mathematically, a GP is a a collection of random variables such that their joint probability distribution is a multivariate Gaussian. In the  context of our application, this collection is the performance metrics for the tuning parameter candidate values. For the initial grid of four samples shown above, the realization of these four random variables were `r knitr::combine_words(round(collect_metrics(svm_initial)$mean, 4))`. The _inputs_ that that define the independent variables/predictors for the GP model are the tuning parameter values: 
...
Gaussian process models are specified by their mean and covariance functions. The latter component has the most effect on the nature of the GP model. The covariance function is often specified in terms of the input values. As an example, a commonly used covariance function is the squared exponential^[This equation is also the same as the _radial basis function_ used in kernel methods, such as the SVM model that is currently being used. This is a coincidence; this covariance function is unrelated to the SVM tuning parameter that we are using. ] function: 

$$cov(y_i, y_j) = exp\left(-\frac{1}{2}|\boldsymbol{x}_i - \boldsymbol{x}_j|^2\right) + \sigma^2_{ij}$$
where $\sigma^2_{ij}$ is a constant error variance term that is zero when $i=j$. This equation translates to 

> "As the distance of two tuning parameter combinations increases, the covariance between the peformance metrics increase exponentially." 

This also means that the variation of the outcome metric is minimized at the points that have already been observed. 

The nature of this covariance function allows the Gaussian process model to represent highly nonlinear functions even when only a small amount of data exists. Fitting these models can be difficult in some cases and the model becomes more computationally expensive as the number of tuning parameter combinations increases. 

Another virtue of this model is that, since a full probability model is specified, the predictions for new inputs can reflect the distribution of the outcome. In other words, the performance values can be predicted in terms of both mean and variance. Suppose that two new tuning parameters were under consideration. In the table below, candidate _A_ has a slightly better mean ROC value than candidate _B_ (the current best is `r round(max(collect_metrics(svm_initial)$mean), 4)`). However, its variance is four-fold larger than _B_. Is this good our bad? It means that it would be more risky to sample but has higher return. The increase in variance also reflects that this new value is further away form the existing data than _B_. The next section considers these aspects of GP predictions for Bayesian optimization in more detail.
...
As previously mentioned, Bayesian optimization is an iterative process. Based on the existing results, the GP model is fit, and new candidates are predicted. We compute performance estimates for the new configuration, the GP is refit with the additional data point (and so on). 

### Acquisition functions

Once the Gaussian process is fit to the current data, how is it used? Our goal is to choose the next tuning parameter combination that is most likely to have "better results" than the current best. One approach to do this, to to create a large candidate set (perhaps using a space-filling design) and then make mean and variance predictions on each.  Using this information, the best tuning parameter value can be chosen. 

A class of objective functions, called _acquisition functions_, are designed to facilitate the trade-off between mean and variance. Recall that the predicted variance of the GP models are mostly driven by how far away they are from the existing data. The trade-off between the predicted mean and variance for new candidates is frequently viewed through the lens of exploration and exploitation: 
...
* _Exploitation_ relies solely on the mean prediction to find the best (mean) value. It focuses on existing results. 
...
set.seed(121)
current_iter <- tibble(x = c(.1, .45, .6, .7, .8)) %>% 
...
For these data, the GP model tightly fits the current data using a highly nonlinear fit for the mean prediction. The shaded region indicates the mean $\pm$ 1 standard error. 
...
gp_pred %>% 
  ...
  geom_vline(xintercept = c(0.25, 0.40), lty = 2, alpha = .5) + 
  geom_ribbon(aes(ymin = .mean -  1 * .sd, ymax = .mean + 1 * .sd), alpha = .1) + 
  labs(y = expression(paste("Estimated ", R^2)), x = "Tuning Parameter")
```

The two vertical lines indicate two candidate points that are examined in more detail below. 

The shaded region in the plot above demonstrates the squared exponential variance function; it becomes very large between points and converges to zero at the existing data points. 

From a pure exploitation standpoint, the best choice would select the parameter value that has the best mean prediction. Here, the would be a value of `r round(gp_pred$x[which.max(gp_pred$.mean)], 3)`, just to the left of the existing best observed point at 0.45. 

As a way to add some exploration, a simple (but not often used) approach is to find the tuning parameter associated with the larges confidence interval. For example, by using a single standard deviation for the $R^2$ confidence bound, the next point to sample is `r round(gp_pred$x[which.max(gp_pred$conf_int_1)], 3)`. This is slightly more into the region with no observed results. Increasing the number of standard deviations used in the upper bound would push the selection further into empty regions. 
...
For example, consider two candidate parameter values of 0.25 and 0.40 (indicated by the vertical lines in the plot above). Using the fitted GP model, their predicted $R^2$ distributions are shown below along with a reference line for the current best results:

```{r iterative-exp-imp, echo = FALSE}
small_pred <- 
  predict(gp, c(0.25, 0.40))$complete_data %>% 
  as_tibble() %>% 
  setNames(c("x", ".mean", ".sd")) %>% 
  mutate(
    value = c(0.25, 0.40),
...
The  $R^2$ distribution for a parameter value of 0.40 has a larger region of values that are more likely to show improvement. As a result, its expected improvement score, `r format(small_pred$exp_imp[small_pred$value == 0.4], digits = 3, scientific = FALSE)`, is larger than the score associated with a candidate value of 0.25 (expected improvement of `r format(small_pred$exp_imp[small_pred$value == 0.25], digits = 3, scientific = FALSE)`). 

When computed across the range of the tuning parameter, the best parameter value to choose is `r gp_pred$x[which.max(gp_pred$exp_imp)]`: 
...
To use Bayesian optimization, use the `tune_bayes()` function. It has syntax that is very similar to `tune_grid()` but with several additional arguments: 

* `iter` is the maximum number of search iterations. 

* `initial` can be either an integer or the results of using `tune_grid()`. For integers, an initial set of results are generated using a space-filling design. 
...
For example, we will use the initial SVM results from Section \@ref(svm) are used as the initial substrate for the Gaussian process model. Recall that, for this application, we want to maximize the area under the ROC curve. Our code is: 
...
if (last_iter < 25) {
  last_bo_text <-
    paste0(
      ...
      max(new_best_iter),
      ", there were no more improvements and the default option is to stop if no improvements are found after `no_improve = ",
      ctrl$no_improve,
...
Also, if the user interrupts the `tune_bayes()` computations, the function returns the current results (instead of resulting in an error).  
...
In our implementation, the neighborhood is determined by scaling the current candidate to be between zero and one based on the range of the parameter object. For this reason, radius values between 0.05 and 0.15 seem reasonable. For these values, fastest that the the search could go from one since of the parameters space to the other is 20 and 10 iterations, respectively. The size of the radius controls how quickly the search explores the parameter space. In our implementation, a range of radii is specified so different magnitudes of "local" define the new candidate values. 
...
Note that, during some iterations, the candidate sets along the radius excludes points outside of the parameter boundaries. Our implementation biases the choice of the next tuning parameter configurations _away_ from new values that are similar to previous configurations. 
...
The syntax for this function are nearly identical to `tune_bayes()`. There are no options for acquisition functions or uncertainty sampling. The `control_sim_anneal()` function has some details that define the local neighborhood and the cooling schedule:

* `no_improve`, for simulated annealing, is an integer that will stop the search if no global best or improved results are not discovered within `no_improve` iterations. Accepted sub-optimal or discarded parameters count as "no improvement".

* `restart` is the number of iterations with no new best results before starting from the previous best results. 

* `radius` is a numeric vector on (0, 1) that defines the minimum and maximum radius that defines the local neighborhood for numeric (non-integer) values. 

* `flip` is a probability value that defines the changes of changing the value of categorical or integer parameters. 
...
This chapter describes two new search methods for optimizing tuning parameters. Both can be effective at finding good values alone or as a follow-up method that is used after an initial grid search to further fine-tune performance. 
