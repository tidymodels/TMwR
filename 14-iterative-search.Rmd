...
To demonstrate, let's look at a toy example with a single parameter that has values between [0, 1] and the performance metric is $R^2$. The true function is shown below in Figure \@ref(fig:performance-profile), along with `r nrow(current_iter)` candidate values that have existing results as points.
