...
bo_stop_index <- grep("Iteration 3", svm_bo_output) 
if (length(bo_stop_index) > 0) {
  cat(svm_bo_output[1:(bo_stop_index - 2)], sep = "")
}
```
...
bo_start <- paste("Iteration", last_iter)
bo_start_index <- grep(bo_start, svm_bo_output)
if (length(bo_start_index) > 0) {
  cat(svm_bo_output[bo_start_index:length(svm_bo_output)], sep = "")
}
```
...
```{r iterative-neighborhood-calcs}
#| echo = FALSE, 
#| message = FALSE, 
#| warning = FALSE,
#| out.width = "80%",
...
ctrl_sa <- control_sim_anneal(no_improve = 10L, verbose_iter = TRUE, save_history = TRUE)
...
if (length(so_stop_index) > 0) {
  cat(svm_sa_output[1:so_stop_index])
}
```
