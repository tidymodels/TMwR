...
next_neighbors <- 
  finetune:::random_real_neighbor(iter_1, glmn_param, r = .1, retain = 100) %>% 
  mutate(Iteration = 1)
...
for (i in 2:6) {
  ...
  next_neighbors <- 
    finetune:::random_real_neighbor(next_value %>% select(-Iteration), glmn_param, r = .1, retain = 50) %>% 
    mutate(Iteration = i)
  neighbors_values <- 
    bind_rows(
      neighbors_values,
...
```
Note that, during some iterations, the candidate sets along the radius excludes points outside of the parameter boundaries.  
