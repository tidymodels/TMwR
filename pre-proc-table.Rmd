...
tkp <- paste0("$\\times^\\dagger$")
cl1 <- paste0("$\\circ^1$")
xp  <- paste0("$\\times^\\dagger$")
...
tab %>% 
  arrange(model) %>% 
  mutate(
    model = paste0("\\verb|", model, "|")
    ) %>% 
  kable(escape = FALSE, booktabs = TRUE, align = c("l", rep("c", ncol(tab) - 1))) %>% 
...
The notation of $^\dagger$ means that the answer depends on the implementation. Specifically: 
