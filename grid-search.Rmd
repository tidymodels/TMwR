...
`r pkg(dials)` contains functions for latin hypercube and maximum entropy designs. As with `grid_random()`, the primary inputs are the number of parameter combinations and a parameters object. Let's compare a random design with a latin hypercube design for 15 candidate parameter values. 
...
The data are included in the `r pkg(modeldata)` package. One column is not needed for analysis (`case`) is removed. 
...
For example, partial least squares (PLS) is a supervised version of principal component analysis REF. It creates components that maximize the variation in the predictors (like PCA) but simultaneously tries to maximize the correlation between these predictors and the outcome. One tuning parameter is the number of PLS components to retain. Suppose that a data set with 100 predictors is fit using PLS. The number of possible components to retain can range from one to fifty. However, in many implementations, a single model fit can compute predicted values across many values of `num_comp`. As a result, a PLS model created with 100 components can also make predictions for any `num_comp <= 100`. The saves time since, instead of creating redundant model fits, a single fit can be used to evaluate many _submodels_.

While not all models have exploit this feature, some do: 
...
For example, if a boosted C5.0 classification model [@apm] was fit to the cell data, we can tune the number of boosting iterations can. With all other parameters set at their default values, iterations from 1 to 100 were evaluated on the same resamples as used previously:
...
As previously mentioned in Section \@ref(parallel), parallel processing is an effective method for creasing training time, especially when models are resampled. This advantage conveys to model tuning with grid search. 
Currently, the `r pkg(tune)` package only parallel processes over resamples (instead of resamples and tuning parameter combinations). The rationale is that this is the optimal scenario when the preprocessing method is expensive. However, it currently limits the achievable speed-ups when the preprocessing is not expensive.  

The multilayer perceptron `tune_grid()` code from Section \@ref(grids) was re-run with different numbers of parallel workers using a computer with 10 physical cores. The results show that parallel process effectively reduces the total execution time for this model: 
...
As another example, the C5.0 classification model shown in the previous section was also run in parallel with ten workers. The parallel computations took 13.3 seconds for a speed-up of `r round(100.147/13.265, 1)`-fold (both runs used the submodel optimization trick). 
...
On issue with grid search is that all models need to be fit across all resamples before any tuning parameters can be evaluated. It would be helpful if, at some point during tuning, an interim analysis is conducted to eliminate any truly awful parameter candidates. This would be akin to _futility analysis_ in clinical trials. If a new drug is performing excessively poorly (or well), it is potentially unethical to wait until the trial finishes to make a decision.  

In machine learning, the set of techniques called _racing method_ provide a similar function [@maron1994hoeffding]. Here, the tuning process evaluates all models on an initial subset of resamples. Based on their current performance metrics, some parameter sets are not considered in subsequent resamples. 
...
```{r grid-mlp-racing, echo = FALSE, fig.height=4}
roc_resamples <- 
  mlp_sfd_tune %>% 
  collect_metrics(summarize = FALSE) %>% 
  filter(id %in% paste0("Fold0", 1:3))
...
roc_resamples$.config <- factor(roc_resamples$.config, levels = config_order)

roc_model <- lmer(.estimate ~ .config + (1|id), data = roc_resamples)

point_est <- 
  summary(roc_model) %>% 
  pluck("coefficients") %>% 
  as_tibble(rownames = ".config") %>% 
  select(.config, estimate = Estimate)

interval_est <- 
  confint(roc_model, quiet = TRUE) %>% 
  as_tibble(rownames = ".config") %>% 
  select(.config, upper = `97.5 %`)

roc_estimates <- 
  inner_join(point_est, interval_est, by = ".config") %>% 
  filter(.config != "(Intercept)") %>% 
  mutate(
    .config = gsub(".config", "", .config, fixed = TRUE),
    .config = factor(.config, levels = rev(config_order))
  )
...
roc_estimates %>% 
  ...
  geom_point(size = 3.2) + 
  geom_errorbarh(aes(xmin = -estimate, xmax = -upper), height = .5) + 
  labs(x = "Drop in ROC AUC from current best candidate", y = NULL)
```

Any parameter set whose confidence interval includes zero would lack evidence that its performance is not statistically different from the best results. We retain `r sum(roc_estimates$upper >= 0)` settings; these are resampled more. The remaining `r sum(roc_estimates$upper < 0)` submodels are no longer considered. 
...
Racing methods can be more efficient that basic grid search as long as the interim analysis is fast and some parameter settings have poor performance. It also is most helpful when the model does not have the abilities to exploit submodel predictions (as previously discussed). 

There are other interim analysis techniques for discarding settings. For example, @krueger15a uses traditional sequential analysis methods whereas @kuhn2014futility treats the data as a sports competition and uses the Bradley-Terry model [@bradley1952rank] to measure the winning ability of parameter settings. The `r pkg(finetune)` package has model tuning functions for several racing methods.  

## Chapter summary {#grid-summary}

We discussed two main classes of grid (regular and non-regular) and demonstrated how to construct them using the family of `grid_*()` functions. The `tune_grid()` function was used to evaluate the candidate sets using resampling. The chapter also showed how to update a model, recipe, or workflow to update the parameter values for the final fit. 
