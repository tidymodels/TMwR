...
* Multivariate adaptive regression splines (MARS) adds a set of nonlinear features features to a linear regression models. The number of terms to retain is a tuning parameter and it is computationally fast to make predictions across many values of this parameter from a single model fit. 

The `r pkg(tune)` package automatically applies this type of optimization whenever an applicable model is tuned. 

For example, if a boosted C5.0 classification model was fit to the cell data, we can tune the number of boosting iterations can. With all other parameters set at their default values, iterations from 1 to 100 were evaluated on the same resamples as used previously:
...
Without the submodel optimization, the call to `tune_grid()` used `r round(3636.685/60, 1)` minutes to evaluate 100 submodels. With the optimization, the same call took 102 _seconds_ (a speedup of `r round(3636.685/101.878, 0)`-fold). The reduced time is the difference in `tune_grid()` fitting 1000 models versus 10 models. 
...
Note that the speed-up in computations will vary from model-to-model. A very computationally efficient model may not benefit as much from parallel processing. 
...
In machine learning, the set of techniques called _racing method_ provide a similar function. Here, the tuning process evaluates all models on an initial subset of resamples. Based on their current performance metrics, some parameter sets are not considered in subsequent resamples. 
...
```
Any parameter set whose confidence interval includes zero would have performance that is not statistically different from the best. We retain `r sum(roc_estimates$upper >= 0)` settings; these are resampled more. The remaining `r sum(roc_estimates$upper < 0)` submodels are no longer considered. 
...
Racing methods can be more efficient that basic grid search as long as the interim analysis is fast and some parameter settings have poor performance. It also is most helpful when the model does not have the abilities to exploit submodel predictions (as discussed in the last section). 
