...
```{r grid-random-matrix, echo = FALSE, fig.height = 6, fig.width = 6}
library(ggforce)
set.seed(200)
mlp_param %>% 
...
```{r grid-sfd-compare, echo = FALSE, fig.height = 6, fig.width = 6}
set.seed(200)
mlp_param %>% 
...
```{r grid-cells-regular, warning = FALSE, cache = TRUE}
...
* Multivariate adaptive regression splines (MARS) add a set of nonlinear features features to a linear regression model [@Friedman:1991p109]. The number of terms to retain is a tuning parameter and it is computationally fast to make predictions across many values of this parameter from a single model fit. 
...
As an example, a boosted tree with the `r pkg(xgboost)` engine was tuned for a data set of 4,000 samples, using 5-fold cross-validation and 10 candidate models. These data required some baseline preprocessing that did not require any estimation. The preprocessing was handled in three different ways:
