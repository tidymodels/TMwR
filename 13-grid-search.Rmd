...
There are high-level convenience functions to understand the results. First, the `autoplot()` method for regular grids shows the performance profiles across tuning parameters in Figure \@ref(fig:regular-grid-plot).  
...
When using tidymodels, it is possible to use values in your local environment (usually the global environment) in model objects. For example, 
...
parsnip models save any arguments as _quosures_; these are objects that track the name of the object as well as where it lives: 

```{r grid-spec-quo}
spec$args$penalty
```

This code works when run in a user's regular session since that session is using the global environment and it can easily find the object `coef_penalty`. 

However, when the model is evaluated with parallel workers, it may fail. Depending on the technology that is used, the parallel workers may not have access to the global environment. 

When writing code that will be run in parallel, it is a good idea to insert the actual data into the objects rather than the reference to the object. The rlang and dplyr package can be very helpful for this. For example, the `!!` operator can splice a single value into an object: 
...
The output of `^0.1` indicates that the value is there instead of the reference to the object. When you have multiple external values to insert into an object, the `!!!` operator can help: 
...
Recipe selectors are another place where one might want access to global variables. Suppose you have a recipe step that should use all of the predictors in the cell data that were measures using the second optical channel. We can create a vector of these column names: 
...
The latter is better for parallel processing since all of the information is embedded in the recipe object. 
