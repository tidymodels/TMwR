...
To illustrate how the parallel processing works, consider a situation with 10-fold cross-validation, a grid of none tuning parameter combinations (all on model parameters), and ten worker processes (i.e., cores). Using this parallel processing scheme, this visualization shows how the tasks are allocated to the worker processes: 

```{r grid-logging-rs, echo = FALSE, message = FALSE}
...
Alternatively, the two nested loops shown above can be _flattened_ or combined into a single loop, represented in this pseudocode: 
...
To illustrate the delegation of tasks to the workers in this scheme: 

```{r grid-logging-all, echo = FALSE, message = FALSE}
times_all  %>%
...
Here multiple folds are allocated within each worker and the preprocessing is repeated. 
...
As an example, a boosted tree with the `xgboost` engine was tune for a data set of 4,000 samples. Five-fold cross-validation was used with 10 candidate models. There was preprocessing that was used but was implemented three different ways: 

1. Prior to modeling using a `dplyr` pipeline (labeled is "none" in the plots below).
2. The same preprocessing but using a recipe (shown as "light" preprocessing).
3. With an additional recipe step that has a high computational cost (labeled as "expensive"). 

The difference between the first and second preprocessing options is designed to measure the computational cost of the recipe. The third option is intended to exploit the problem of performing redundant computations with `parallel_over = "everything"`. 
...
When the expensive preprocessing step is added, there is a considerable difference in execution times. Using `parallel_over = "everything"` is problematic since, even using all cores, it never achieves the execution time that `parallel_over = "resamples"` attains with five cores. This is because the costly preprocessing step is unnecessarily repeated in the computational scheme. 
