...
```{r grid-mlp-racing, echo = FALSE, fig.height=5}
roc_resamples <- 
  mlp_sfd_tune %>% 
...
roc_estimates <- 
  ...
  mutate(
    ...
    Results = ifelse(upper < 0, "eliminate", "retain"),
    Results = factor(Results, levels = c("retain", "eliminate"))
  )
race_cols <- c(retain = "black", eliminate = "darkgrey")

roc_estimates %>% 
  ggplot(aes(x = -estimate, y = .config)) + 
  geom_vline(xintercept = 0, lty = 2, col = "green") +
  geom_point(size = 3.2, aes(col = Results)) + 
  geom_errorbarh(aes(xmin = -estimate, xmax = -upper, col = Results), height = .5, show.legend = FALSE) + 
