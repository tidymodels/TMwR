...
```{r grid-logging-rs, echo = FALSE, message = FALSE, fig.height = 9}
...
In this case, parallelization now occurs at line 3. For example, if we use 10-fold cross-validation with $M$ tuning parameter values, the loop is executed over $10\times M$ iterations. This increases the number of potential workers that can be used. However, the work in lines 4-5 (related to data processing) are _repeated_ multiple times. If those steps are expensive, this approach will be  inefficient. 

To illustrate the delegation of tasks to the workers in this scheme, the same example is used but with 12 workers:

```{r grid-logging-all, echo = FALSE, message = FALSE, fig.height = 9}
times_all  %>%
