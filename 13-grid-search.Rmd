...
Let's consider two different prallel processing schemes. 

When tuning models via grid search, the process involves two distinct loops: one over resamples and another over the unique tuning parameter combinations. Representing this process with pseudocode: 
...
To illustrate how the parallel processing works, consider a situation with 10-fold cross-validation, a grid of eight tuning parameter combinations (all on model parameters), and ten worker processes (i.e., cores). 

Using the scheme that parallelizes resamples, this visualization shows how the tasks are allocated to the worker processes: 

```{r grid-logging-rs, echo = FALSE, message = FALSE, fig.height = 7}
load("extras/parallel_times/logging_data.RData")
times_resamples %>%
  ggplot(aes(x = seconds, y = id, col = operation)) +
  ...
  labs(y = NULL, x = "Elapsed Time (s)") + 
  scale_color_brewer(palette = "Dark2")
```

Note that each fold is assigned to its own worker process and, since only model parameters are being tuned, the preprocessing is conducted once per fold/worker. If fewer than 10 worker processes were used, some workers would receive multiple folds. 
...
In this case, parallelization now occurs at line 11. For example, if we use 10-fold cross-validation with $M$ tuning parameter values, the loop is executed over $10\times M$ iterations. This increases the number of potential workers that can be used. However, the work in lines 12-13 (related to data processing) is _repeated_ multiple times. If those steps are expensive, this approach will be  inefficient. 

To illustrate the delegation of tasks to the workers in this scheme, the same example is used but with 12 workers:

```{r grid-logging-all, echo = FALSE, message = FALSE, fig.height = 7}
times_all  %>%
  ggplot(aes(y = id, x = seconds, col = operation)) +
  geom_point(pch = "|", size = 4) +
  facet_wrap(~ pid) +
  theme_bw() +
  labs(y = NULL, x = "Elapsed Time (s)") +
...
As an example, a boosted tree with the `xgboost` engine was tune for a data set of 4,000 samples. Five-fold cross-validation was used with 10 candidate models. These data required some baseline preprocessing that did not require any estimation. The preprocessing was handled three different ways:
...
The best speed-ups, for these data, occur when `parallel_over = "resamples"` and when the computations are expensive. However, in the latter case, the previous analysis indicates that the overall model fits are slower.  

What is the benefit of using the submodel optimization method _in conjunction with parallel processing_?  The C5.0 classification model shown in the previous section was also run in parallel with ten workers. The parallel computations took 13.3 seconds for a speed-up of `r round(100.147/13.265, 1)`-fold (both runs used the submodel optimization trick). Between the submodel optimization trick and parallel processing, there was a __total speed-up of `r round(3734.249/13.265, 0)`-fold__ over the most basic grid search code. 

Overall, it is important to note that the increased computational savings will vary from model-to-model and is also affected by the size of the grid, the number of resamples, etc. A very computationally efficient model may not benefit as much from parallel processing. 
