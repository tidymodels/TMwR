...
If the initial pool of data is not huge, there will be some overlap of usage and a solid methodology for data spending is important. This chapter demonstrates the basics of _data splitting_ our initial pool of samples for different purposes. 
...
The other portion of the data are placed into the _test set_. This is held in reserve until one or two models are chosen as the best methods. The test set is then used as the final arbiter to determine the efficacy of the model. It is critical to only look at the test set once; otherwise it becomes part of the modeling process. 

How should we conduct this split of the data? This depends on the context. Suppose 75% of the data are allocated to the training set and the remaining 25% is used for testing.  The most common method is to use simple random sampling. The `rsample` package has tools for making data splits and the function `intial_split()` was created for this purpose. It takes the data frame as an argument as well as the proportion to be placed into training. For example: 
...
The printed information denotes the amount of data in the training set ($n = `r nrow(training(ames_split))`$), the amount in the test set ($n = `r nrow(testing(ames_split))`$), and the size of the original pool of samples  ($n = `r nrow(ames)`$). The object `ames_split` only contains the splitting information; to get separate data objects, two more functions are applied:
...
The resulting objects are data frames with the same columns as the original data but only contain the appropriate rows. 
