...
## Tools for creating model specifications

There are a few tools that can help uses create models. 
...
use_xgboost(Sale_Price ~ Neighborhood + Gr_Liv_Area + Year_Built + Bldg_Type + 
              Latitude + Longitude, 
            data = ames_train,
            # Don't print the model tuning code:
            tune = FALSE,
...
This code is, based on the little that `r pkg(usemodels)` understands about the data, the minimal preprocessing required. For other models, operations like `step_normalize()` are added to fulfill the basic needs of the model. 

If there is interest in trying a large number of models, it may become tedious to write many model specifications. `r pkg(parsnip)` includes an [RStudio addin](https://rstudio.github.io/rstudioaddins/) that can help. Running the code: 

```{r models-add-in, eval = FALSE}
parsnip::write_parsnip_specs()
```
