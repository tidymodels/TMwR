...
Don't reevaluate the training set; empirical validation using out-of-sample data
...
two_factor_res <- 
  ...
  pull(mean)
```
Model effectiveness is probably defined by how the model will be used. For inferential models, there is strong focus on the choice (and validity) of probabilistic distributions and other generative qualities that define the model. If the model is solely used for prediction, there is less importance placed on statistical validity in favor or predictive strength. Since all models are somewhat reductive, predictive strength is usually focused on how close our predictions come to the observed data, i.e., how much fidelity to the predictions have to the actual results. 

This chapter focuses on functions that can be used to measure predictive strength. However, our advise for those developing inferential models is to use these techniques _even when the model will not be used for prediction_. A long-standing issue with the practice of inferential statistics is that, when the focus is purely on inference, it is difficult to tell how credible the model truly is. For example, consider the data from @CraigSchapiro when `r nrow(ad_data)` patients were studied to determine the factors that influence cognitive impairment which, in these data, is focused on Alzheimer's disease. An analysis might take the known risk factors and add them to a logistic regression model where the outcomes  binary (impaired/non-impaired). We'll use predictors for age, sex, and  Apolipoprotein E genotype. The latter is a categorical variable with the six possible combinations of the three main variants of this gene. Apolipoprotein E is know to have an association with dementia [@Kim:2009p4370].

A superficial, but not uncommon, approach to this analysis would be to fit a large model with main effects and interactions, then use statistical tests to find the minimal set of predictors that are statistically significant at some pre-defined level. If a full model with the three factors and their two- and three-way interactions were used, an initial phase would be to test the interactions using sequential likelihood ration tests [@HosmerLemeshow].  For example, when comparing the model with all two-way interactions to one with the additional three-way interaction, the likelihood ration tests produces a p-value of `r three_factor_test[2, "Pr(>Chi)"]`. This implies that there is no evidence that the `r abs(three_factor_test[2, "Df"])` additional model terms required for the three-way interaction explains enough of the variation in the data to be retained in the model. Next, the two-way interaction are similarly evaluated against the model with no interactions. The p-value here is `r two_factor_test[2, "Pr(>Chi)"]`. This is somewhat borderline but, given the small sample size, it would be prudent to conclude that there is evidence that some of the `r abs(two_factor_test[2, "Df"])` two-way interactions are important to explain the outcome. 

From here, an explanation of the results would occur The interactions would be particularly important to discuss since that may sparks interesting physiological or neurological hypotheses to be explored further. 

While admittedly superficial, this analysis strategy is fairly common in practice as well as in the literature. This is especially true if the practitioner has limited formal training in data analysis. 

Importantly, one missing piece of information is related to how closely does this model fits the actual data. Using resampling methods, discussed in Chapter \@ref(resampling), we can estimate the accuracy of this model is about `r round(two_factor_res * 100, 1)`%. It should be said that accuracy is a poor measure of model performance; it is used here since it is commonly understood. If the data has `r round(two_factor_res * 100, 1)`% fidelity to the data, should we trust the conclusions produced by the model? One might think so until they realize that the baseline rate of non-impaired patients in the data is `r round(mean(ad_data$Class == "Control") * 100, 1)`%. This means that, despite our statistical analysis, the two-factor model appears to be _only `r round((two_factor_res - mean(ad_data$Class == "Control")) * 100, 1)`% better than a simple model that always predicts patients to be unimpaired_, irregardless of the observed data. 
...
There are methods for using metrics that are specific to cases with two classes for data sets with more than two classes. For example, a metric such as sensitivity measures the true positive rate which, by definition, is specific to two classes (i.e., "event" and "non-event"). How would this metric be used in general? 

To applied two-class metrics, there are wrapper methods that can be used. Three  options are macro-, weighted macro-, and micro-averaging: 
