...
Once we have a model there is a need to know how well it works. Having a quantitative approach for estimating effectiveness allows us to understand the model, compare different models, or to tweak the model to improve performance. The focus in tidymodels is on _empirical validation_; this usually means using data that were not used to create the model as the substrate to measure effectiveness. The best approach to empirical validation involves using _resampling_ methods that will be introduced in Chapter \@ref(resampling). Until then, the test set will be (inappropriately) used for illustration. 

The choice of the metrics to examine can be critical. In later chapters, certain model parameters will be empirically optimized and a primary performance metric will be used to choose the best _sub-model_. Choosing the wrong method can easily result in unintended consequences. For example, the two common metrics for regression models are the root mean squared error (RMSE) and the coefficient of determination (a.k.a. $R^2$). The former measures _accuracy_ while the latter measures _correlation_. These are not necessarily the same thing. This figure demonstrates the difference between the two: 
...
This chapter will largely focus on the `yardstick` package. Before illustrating syntax, there is a short discussion about whether emprical validation using performance metrics is worthwhile when a model is focused on inference rather than prediction. 

## Performance metrics can certify inferential results
...
two_factor_res <- 
  ...
  pull(mean)
```
Model effectiveness is probably defined by how the model will be used. For inferential models, there is strong focus on the choice (and validity) of probabilistic distributions and other generative qualities that define the model. If the model is solely used for prediction, there is less importance placed on statistical validity in favor of predictive strength. Since all models are somewhat reductive, predictive strength is usually focused on how close our predictions come to the observed data, i.e., fidelity of the model predictions to the actual results. 

This chapter focuses on functions that can be used to measure predictive strength. However, our advise for those developing inferential models is to use these techniques _even when the model will not be used for prediction_. A long-standing issue with the practice of inferential statistics is that, when the focus is purely on inference, it is difficult assess the crediblity of the model. For example, consider the data from @CraigSchapiro when `r nrow(ad_data)` patients were studied to determine the factors that influence cognitive impairment which, in these data, is focused on Alzheimer's disease. An analysis might take the known risk factors and add them to a logistic regression model where the outcomes  binary (impaired/non-impaired). We'll use predictors for age, sex, and  Apolipoprotein E genotype. The latter is a categorical variable with the six possible combinations of the three main variants of this gene. Apolipoprotein E is known to have an association with dementia [@Kim:2009p4370].

A superficial, but not uncommon, approach to this analysis would be to fit a large model with main effects and interactions, then use statistical tests to find the minimal set of model terms that are statistically significant at some pre-defined level. If a full model with the three factors and their two- and three-way interactions were used, an initial phase would be to test the interactions using sequential likelihood ratio tests [@HosmerLemeshow].  For example, when comparing the model with all two-way interactions to one with the additional three-way interaction, the likelihood ratio tests produces a p-value of `r three_factor_test[2, "Pr(>Chi)"]`. This implies that there is no evidence that the `r abs(three_factor_test[2, "Df"])` additional model terms associated with the three-way interaction explain enough of the variation in the data to keep in the model. Next, the two-way interaction are similarly evaluated against the model with no interactions. The p-value here is `r two_factor_test[2, "Pr(>Chi)"]`. This is somewhat borderline but, given the small sample size, it would be prudent to conclude that there is evidence that some of the `r abs(two_factor_test[2, "Df"])` possible two-way interactions are important to the model. 

From here, an explanation of the results would occur. The interactions would be particularly important to discuss since they may sparks interesting physiological or neurological hypotheses to be explored further. 

While admittedly shallow, this analysis strategy is fairly common in practice as well as in the literature. This is especially true if the practitioner has limited formal training in data analysis. 

Importantly, one missing piece of information is related to how closely does this model fits the actual data. Using resampling methods, discussed in Chapter \@ref(resampling), we can estimate the accuracy of this model to be about `r round(two_factor_res * 100, 1)`%. It should be said that accuracy is a poor measure of model performance; it is used here since it is commonly understood. If the data has `r round(two_factor_res * 100, 1)`% fidelity to the data, should we trust the conclusions produced by the model? One might think so until they realize that the baseline rate of non-impaired patients in the data is `r round(mean(ad_data$Class == "Control") * 100, 1)`%. This means that, despite our statistical analysis, the two-factor model appears to be _only `r round((two_factor_res - mean(ad_data$Class == "Control")) * 100, 1)`% better than a simple model that always predicts patients to be unimpaired_, irregardless of the observed data. 

```{block, type = "rmdnote"}
The point of this analysis is to demonstrate the idea **optimization of statistical characteristics of the model does not imply that the model fits the data well.**   For purely inferential models, some measure of fidelity to the data should accompany the inferential results. Using this, the consumers of the analyses can calibrate their expectations of the results of the statistical analysis. 
```
...
Recall that the tidymodels prediction functions produce tibbles that include columns for the predicted values. These columns have consistent names. The functions in the `yardstick` package that produce performance metrics have consistent interfaces. The functions are data frame-based, as opposed to vector-based, with the general syntax of 
...
```
The output above shows the standard format of the output of `yardstick` functions. Metrics for numeric outcomes usually have a value of "standard" for the `.estimator` column. Examples with different values of this column are shown below.

If there is interest in multiple metrics, a _metric set_ can be created so that a single function can be used. Let's add $R^2$, and the mean absolute error: 
...
`yardstick` does _not_ contain a function for adjusted $R^2$. This commonly used modification of the coefficient of determination is needed when the same data used to fit the model are used to evaluate the model. It is somewhat deprecated when using tidymodels because it is always a better approach to compute performance on a separate data set than the one used to fit the model.  
...
```{block, type = "rmdnote"}
There is some heterogeneity in R functions in this regard; some use the first level and others the second to denote the event of interest. Logically, it would be more intuitive that the first level is the most important. Unfortunately, the second level logic is borne of encoding the outcome as 0/1 (in which case the second value is the event). However, tidymodels (and many R packages) _require_ a categorical outcome to be encoded as a factor and, for this reason, the legacy justification for the second level as the event becomes irrelevant.  
```
...
The `two_class_curve` object can be used in a `ggplot` call to visualize the curve. There is also an `autoplot()` method that will take care of the details:
...
Note that, in these results, a "multiclass" `.estimator` is listed. Like "binary", this indicates that the formulas for outcomes with three or more class levels was used. The  Matthews correlation coefficient was originally designed for two classes but was extended to cases with more class levels. 

There are methods for using metrics that are specific to outcomes with two classes for data sets with more than two classes. For example, a metric such as sensitivity measures the true positive rate which, by definition, is specific to two classes (i.e., "event" and "non-event"). How would this metric be used in our example data? 

There are wrapper methods that can be used to apply sensitivity to our four-class outcome. These options are macro-, weighted macro-, and micro-averaging: 
...
but macro-averaging is also available: 
