...
The default range may not have served us well here. To evaluate the  meta-learning model with smaller penalties, let's pass an additional option:


```{r ensembles-second-blend, cache = TRUE}
set.seed(2001)
ens <- blend_predictions(concrete_stack, penalty = 10^seq(-4, -0.5, length = 30))
autoplot(ens)
```
...
```{r artifacts, include = FALSE}
save.image(file = "ensembles.RData", version = 2, compress = TRUE)
```
