...
stacks() %>% 
  ...
  kable_styling("striped", full_width = TRUE, latex_options = "HOLD_position") %>% 
...
The top panel shows the average number of candidate ensemble members retained by the meta-learning model. We can see that the number of members is fairly constant and, as it increases, the RMSE also increases. The R<sup>2</sup> values increase with larger penalties. This is somewhat counter-intuitive, but the y-axis range of each panel reminds us that these changes are minuscule. 
...
This is slightly better than our best single model. It is fairly common for stacking to produce incremental benefits when compared to the best single model. 
