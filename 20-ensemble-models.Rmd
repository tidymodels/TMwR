...
# Get a subset of column names to make an abbreviated table
cand_names <- names(res)
cart_bag_col <- grep("^CART_bagged", cand_names, value = TRUE)[1]
cubist_cols <- grep("^Cubist", cand_names, value = TRUE)
cubist_first_col <- cubist_cols[1]
cubist_last_col <- cubist_cols[length(cubist_cols)]

res |> 
  select(
    sample_num, all_of(cart_bag_col), starts_with("MARS"), 
    all_of(cubist_first_col), buffer_1, all_of(cubist_last_col), buffer_2
  ) |> 
  knitr::kable(
    digits = 2,
    align = rep("c", 8),
