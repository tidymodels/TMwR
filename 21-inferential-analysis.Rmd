...
#| echo = FALSE,
#| out.width = "80%",
#| fig.cap = "The distribution of the number of articles written within 3 years of graduation.",
...
The `r pkg(infer)` package has a very high-level API for showing the results for the analysis. The foloowing code produces Figure \@ref(fig:bootstrapped-mean):
...
The visualization code is also very similar to the bootstrap approach. The code below generates Figure \@ref(fig:permutation-dist) where a red line signifies the observed value:
...
#| echo = FALSE,
#| out.width = "80%",
#| fig.cap = "The empirical distribution of the test statistic under the null hypothesis. The vertical line indicates the observed test statistic.",
...
```{r inference-zip-comp, cache = TRUE}
...
It's a good idea to visualize the bootstrap distributions of the coefficients. This code produces the image in Figure \@ref(fig:zip-bootstrap):
