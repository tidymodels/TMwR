...
A basic application of the test is:^[The `T` argument allows us to account for how there are more men in the sample to start with.]
...
The `r pkg(infer)` package has a high-level API for showing the results of the analysis, as shown in Figure \@ref(fig:bootstrapped-mean).
...
The visualization code is also very similar to the bootstrap approach. The code below generates Figure \@ref(fig:permutation-dist) where the vertical line signifies the observed value.
...
Since the vertical line in Figure \@ref(fig:permutation-dist) is far away from the permutation distribution (which represents the null hypothesis), the likelihood of observing data that is at least as extreme as what is at hand if in fact the null hypothesis were true is exceedingly small.
...
For count data, there are occasions where the number of zero counts is larger than what a simple Poisson distribution would prescribe. A more complex model appropriate in this situation is the zero-inflated Poisson (ZIP) model; see @Mullahy, @Lambert1992, and @JSSv027i08. Here, there are two sets of covariates: one for the count data and others that affect the probability (denoted as $\pi$) of extra zeros. The equation for the mean $\lambda$ is:
...
and the $x$ covariates affect the count values while the $z$ covariates influence the probability of an extra zero count. The two sets of predictors do not need to be mutually exclusive.
