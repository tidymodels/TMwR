...
For count data, there are occasions where the number of zero counts is larger than what a simple Poisson distribution would prescribe. A more complex model appropriate in this situation is the zero-inflated Poisson (ZIP) model; see @Mullahy, @Lambert1992, and @JSSv027i08. Here, there are two sets of covariates: one for the count data and others that affect the probability of zeros (denoted as $\pi$). The mean function is

$$\lambda = \pi + (1 - \pi) \lambda_{nz}$$
...
where the $x$ covariates affect the count values and the $z$ covariates influence the probability of a zero count. The two sets of predictors do not need to be mutually exclusive.
