...
There is a much nicer user interface to fit models that will be introduced in Section \@ref(workflow-set).  
...
Let's say that we've concluded our exploratory work and have settled on a final model. There is a convenience function called `last_fit()` that will fit the model to the entire training set then evaluate it with the testing set. 

Using `lm_wflow` as an example, we can pass the model information and the initial training/testing split to the function:
...
The `.workflow` column contains the _fitted workflow_ and can be pulled out of the results using

```{r extract-last-fit, eval = FALSE}
fitted_lm_wflow <- extract_workflow(final_lm_res)
```

Similarly, `collect_metrics()` and `collect_predictions()` give easy access to the performance statistics and predictions, respectively. 
