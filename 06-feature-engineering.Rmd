...
The function `all_nominal_predictors()` captures the names of any _predictor_ columns that are currently factor or character (i.e., nominal) in nature. This is a `r pkg(dplyr)` selector function similar to `starts_with()` or `matches()` but can only be used inside of a recipe. 
