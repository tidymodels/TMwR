...
This produces Figure \@ref(fig:workflow-sets-autoplot). 
...
The same helpful functions are available for this object to interrogate the results and, in fact, the basic `autoplot()` method shown in Figure \@ref(fig:workflow-set-racing-ranks) produces similar trends to Figure \@ref(fig:workflow-sets-autoplot). This is produced by:
...
matched_results %>% 
   ...
   # Added to black and white images:
   geom_text_repel(aes(label = model))
```

The results are shown in Figure \@ref(fig:racing-concordance).
...
The test set results are produced using:
...
The results are in Figure \@ref(fig:concrete-test-results):
