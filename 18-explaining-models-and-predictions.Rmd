...
#| echo = FALSE,
#| fig.cap = "Shapley additive explanations for the random forest model for a duplex property.",
#| fig.alt = "Shapley additive explanations for the random forest model for a duplex property. Gross living area and year built have the largest contributions."
```
...
```{r gilbert-shap}
#| echo = FALSE,
#| fig.cap = "Shapley additive explanations for the random forest model for a one-family home in Gilbert.",
#| fig.alt = "Shapley additive explanations for the random forest model for a one-family home in Gilbert. Gross living area and year built have the largest contributions but in the opposite direction as the previous explainer."
shap_house %>%
  group_by(variable) %>%
  mutate(mean_val = mean(contribution)) %>%
  ungroup() %>%
...
#| fig.alt = "Global explainer for the random forest and linear regression models. For both models, gross living area and year built have the large contributions although the linear model uses the neighborhood predictor to a large degree."
...
Using this function generates Figure \@ref(fig:year-built).
...
#| fig.alt = "Partial dependence profiles for the random forest model focusing on the year built predictor. The profiles are relavely constant and then increase linearly around 1950."
```
...
to produce Figure \@ref(fig:building-type-profiles).
...
We have the option of using `plot(pdp_liv)` for default `r pkg(DALEX)` plots, but since we are making plots with the underlying data here, we can even facet by one of the features to visualize how the predictions change differently, highlighting the model's behavior for subgroups (as shown in Figure \@ref(fig:building-type-facets)).
...
The previously defined `ggplot_imp()` function produces Figure \@ref(fig:bean-explainer).

```{r explain-bea-vip, cache = TRUE, results = 'hide'}
vip_beans <- 
  explain_tidymodels(
    rda_wflow_fit, 
  ...
  model_parts() 
```
```{r explain-bea-vip-plot, eval = FALSE}
