...
We'd like to resample each of these models in turn and do to so, a `r pkg(purrr)`-like function called `workflow_map()` is used. This function takes an initial argument of the function to apply to the workflows, followed by an options to that function. We also set a `verbose` argument that will print the progress as well as a `seed` argument that makes sure that each model uses the same random number stream as the others. 
...
To illustrate, let's gather the individual resampling statistics for the linear models and the random forest. We will focus on the R<sup>2</sup> statistic for each model, which measures correlation. 
...
The `X1`, `X2` amd `X3` columns in the table are indicators based on the values in the `model` column. The order of these was defined in the same way R defines them (alphabetically ordered by `model`).  

For our model comparison, the specific ANOVA model is: 

$$y_{ij} = \beta_0 + \beta_1x_{i1} + \beta_2x_{i2} + \beta_2x_{i3} + \epsilon_{ij}$$
...
$$y_{ij} = (\beta_0 + b_{i}) + \beta_1x_{i1} + \beta_2x_{i2} + \beta_2x_{i3} + \epsilon_{ij}$$
...
These histograms describe the estimated probability distributions of the mean R<sup>2</sup> value for each model. There is some overlap, especially for the two linear models. There is also a basic `autoplot()` method for the model results as well as the tidied object that shows overlaid density plots. 
show default autoplot
...
autoplot example with workflows

```{r compare-rope-plot}
autoplot(rsq_anova, type = "ROPE", size = 0.01)
```
