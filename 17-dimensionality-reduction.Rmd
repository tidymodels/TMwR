...
In addition to the `r pkg(tidymodels)` package, this chapter uses the following packages: `r pkg(baguette)`, `r pkg(beans)`, `r pkg(bestNormalize)`, `r pkg(corrplot)`, `r pkg(discrim)`, `r pkg(embed)`, `r pkg(ggforce)`, `r pkg(klaR)`, `r pkg(learntidymodels)`, `r pkg(mixOmics)`[^mixnote], and `r pkg(uwot)`. The `r pkg(learntidymodels)` package can be found at its [GitHub site](https://github.com/tidymodels/learntidymodels).
