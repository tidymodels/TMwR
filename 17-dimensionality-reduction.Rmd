...
[^learnnote]: The `r pkg(learntidymodels)` package can be found at its [GitHub site](https://github.com/tidymodels/learntidymodels) (`https://github.com/tidymodels/learntidymodels`)

[^mixnote]: The `r pkg(mixOmics)` package is not available on CRAN, but instead [on Bioconductor](https://doi.org/doi:10.18129/B9.bioc.mixOmics) (`https://doi.org/doi:10.18129/B9.bioc.mixOmics`).
...
#| echo = FALSE, 
#| out.width="95%",
#| fig.cap = "Example shapes and their eccentricity statistics.",
#| fig.alt = "Example shapes and their eccentricity statistics. Circles and squares have the smallest eccentricity values while X values and lightening bolts have the largest. The eccentricity is also the same when shapes are rotated."
...
library(corrplot)
tmwr_cols <- colorRampPalette(c("#91CBD765", "white", "#CA225E"))
bean_train %>% 
  select(-class) %>% 
  cor() %>% 
...
#| fig.width=6, 
#| out.width="70%",
#| fig.cap = "A correlation matrix of the predictors with variables ordered via clustering.",
...
Figure \@ref(fig:recipe-process) summarizes. Let's look at each of these functions in more detail. 
...
#| out.width = '80%', 
#| warning = FALSE,
#| fig.cap = "A summary of the recipe-related functions.",
...
#| fig.height = 5,
#| fig.width = 5.1,
#| fig.cap = "The first few principal component scores for the bean validation set, colored by class.",
#| fig.alt = "The first few principal component scores for the bean validation set, colored by class. The classes separate when the first to components are plotted against one another."
```
...
#| fig.alt = "Predictor loadings for the PCA transformation. For the first component, the major axis length, second shape factor, convex area, and area have the larges effect. "
```
...
#| fig.height = 5,
#| fig.width = 5.1,
#| fig.cap = "The first few PLS component scores for the bean validation set, colored by class.",
#| fig.alt = "The first few PLS component scores for the bean validation set, colored by class. The first two PLS components are nearly identical to the first two PCA components."
```

The first two PLS components are nearly identical to the first two PCA components! We find this result because those PCA components are so effective at separating the varieties of beans. The remaining components are different. Figure \@ref(fig:pls-loadings) is produces using:
...
#| fig.alt = "Predictor loadings for the PLS transformation. For the first component, the major axis length, second shape factor, the equivalent diameter, convex area, and area have the larges effect. "
```
...
ICA is slightly different than PCA in that it finds components that are as statistically independent from one another as possible (as opposed to being uncorrelated). It can be thought of as maximizing the "non-Gaussianity" of the ICA components. `step_ica()` is used to produce Figure \@ref(fig:bean-ica):
...
#| fig.height = 5,
#| fig.width = 5.1,
#| fig.cap = "The first few ICA component scores for the bean validation set, colored by class.",
#| fig.alt = "The first few ICA component scores for the bean validation set, colored by class. There is significant overlap in the first two ICA components."
```
...
The supervised method, shown on the right side of Figure \@ref(fig:bean-umap), looks promising for modeling the data.

```{r bean-umap, echo = FALSE}
#| dev = "png",
#| fig.height = 5,
#| fig.width = 10.1,
#| fig.cap = "The first few UMAP component scores for the bean validation set, colored by class. Results are show for supervised and unsupervised versions.",
#| fig.alt = "The first few UMAP component scores for the bean validation set, colored by class. Results are show for supervised and unsupervised versions. There are clusters that are extremely separated form one another but each contains a mixture of the classes. The supervised version shows more separation between classes."
...
```{r dimensionality-workflows, cache = TRUE, message = FALSE, warning = FALSE}
...
rankings %>% 
  ...
  labs(y = "ROC AUC") +
  # Added to help with back and white images:
