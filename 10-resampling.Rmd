...
:::rmdnote
Note that _hyperthreading_ is available for most Intel CPUs. This means that, even though your computer has `X` CPU cores, it is designed to do `2X` computations at once. While the speed-ups are larger for the first `X` cores, using the extra capacity can also decrease computational time. To determine how many physical cores are available, use `parallel::detectCores()`. To determine if there are extra "virtual" or "logical" cores to be used, there is an extra argument: `detectCores(logical = TRUE)`. 
:::
