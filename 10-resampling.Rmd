...
if (!file.exists("RData/resampling.RData") |
    !file.exists("RData/resampling.RData") |
    !file.exists("RData/resampling.RData")) {
...
For computations conducted on a single computer, the number of possible worker processes is best determined by the `r pkg(parallelly)` package: 
...
For `fit_resamples()` and other functions in `r pkg(tune)`, parallel processing occurs when the user chooses the parallel technology: 
...
This instructs `fit_resamples()` to run half of the computations on each core. To reset the computations to sequential processing: 

```{r}
#| label: resampling-future-stop
#| eval: false
...
get_model <- function(x) {
  library(broom) # Required for `tidy()` method
