...
is_new_version <- function(x, ...) TRUE
...
For `fit_resamples()` and other functions in `r pkg(tune)`, parallel processing occurs using one of two R frameworks for parallel processing. The `r pkg(future)`  package can be used via its `plan()` function. For example, to use your computer with a local "cluster", running `plan(multisession, workers = X)` will automatically tell the tidymodels packages to use this framework to process work simultaneously (using `X` subprocesses). Revert to sequential processing by using `plan(sequential)`. Alternatively, the  `r pkg(mirai)` package can do the same, with a different syntax. Here, using the `daemons(n = X)` will enable parallel processing, and `daemons(0)` turns it off. Each of these frameworks can be used in conjunction with more complex schemes, such as utilizing an external high-performance computing platform or other setups. 

:::rmdnote
Note that _hyperthreading_ is available for most Intel CPUs. This means that, even though your computer has `X` CPU cores, it is designed to do `2X` computations at once. While the speed-ups are larger for the first `X` cores, using the extra capacity can also decrease computational timeat a lesser rate. To determine how many physical cores are available, use `parallel::detectCores()`. To determine if there are extra "virtual" or "logical" cores to be used, there is an extra argument: `detectCores(logical = TRUE)`. 
:::
