...
When this function is executed, the data are converted from a data frame to a numeric _design matrix_ (also called a _model matrix_) and then the least squares method is used to estimate parameters. Recall that Section \@ref(formula) listed the multiple purposes of the R model formula. Focusing on the data manipulation aspects, the formula above can be decomposed into a series of _steps_:
...
```{block, type = "rmdnote"}
However, as will be shown in Chapter \@ref(workflows), there are high level functions that will handle the second two phases automatically. In these cases, the user would not have to manually use `prep()`, `bake()`, or `juice()` to include a recipe in the modeling process. 
```
