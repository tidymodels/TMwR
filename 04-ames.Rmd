...
While not perfect, this will probably result in better models than using the untransformed data, for the reasons outlined above. 
...
We can see a few noticeable patterns. While there are a number of neighborhoods that are geographically isolated, there are others that are adjacent to each other (or even overlap). For example, as Figure \@ref(fig:ames-timberland) shows, Timberland is located apart from almost all other neighborhoods.
