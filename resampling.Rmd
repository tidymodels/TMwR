...
Using the same predictor set as the linear model (without the extra pre-processing steps), a random forest is fit to the training set using via the `ranger` package. This model requires no preprocessing so a simple formula can be used:
