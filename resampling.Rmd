...
`r round(check_rf(rf_fit_res, ames_test)$.estimate[[1]]/check_rf(rf_fit_res, ames_train)$.estimate[[1]], 1)`)-fold off of test set. 
...
```{r resampling-three-cv, echo = FALSE, out.width = '40%'}
if (knitr:::is_html_output()) {
...
```{r resampling-three-cv-iter, echo = FALSE, out.width = '42%'}
if (knitr:::is_html_output()) {
...
```{r resampling-bootstraps, echo = FALSE, out.width = '50%'}
if (knitr:::is_html_output()) {
...
## Between-model comparisons {#compare}
...
lm_rmse <- 
  ...
  filter(.metric == "rmse") %>% 
  select(id, lin_reg = .estimate)

rf_rmse <- 
  collect_metrics(rf_res, summarize = FALSE) %>% 
  filter(.metric == "rmse") %>% 
  select(id, rand_forest = .estimate)  

rmse_estimates <- inner_join(lm_rmse, rf_rmse, by = "id")
rmse_estimates

cor.test(rmse_estimates$lin_reg, rmse_estimates$rand_forest) %>% 
  tidy() %>% 
  select(estimate, starts_with("conf"))
```


```{r resamples-rmse-plot}
rmse_estimates %>% 
  pivot_longer(cols = c(-id), names_to = "model", values_to = "rmse") %>% 
  ggplot(aes(x = model, y = rmse, group = id, col = id)) + 
  geom_line(alpha = .5, lwd = 1.5) + 
  theme(legend.position = "none")
```


```{r resamples-compare}
t.test(rmse_estimates$lin_reg, rmse_estimates$rand_forest, paired = TRUE) %>%
  tidy()
```
