...
As we discussed in Chapter \@ref(workflows), there are high level functions that handle the second two phases automatically. Our previous workflow (`lm_wflow`) used a simple set of `r pkg(dplyr)` selectors. Instead of that approach, the `simple_ames` recipe is used to preprocess data for modeling.
...
Data are given to recipes at different stages. 

First, when calling `recipe(..., data)`, the data set is used to determine the data types of each column so that selectors such as `all_numeric()` can be used. 

Second, when preparing the data using `fit(workflow, data)`, the training are used for all estimation operations, from determining factor levels to computing PCA components and everything in between. It is important to realize that all preprocessing and feature engineering steps only utilize the training data. Otherwise, information leakage can negatively impact the model. 

Finally, using `predict(workflow, new_data)`, no quantities are re-estimated using the values in `new_data`. Take centering and scaling using `step_normalize()` as an example. Using this step, the means and standard deviations from the appropriate columns are determined from the training set; new samples are standardized using these values when `predict()` is invoked. 
...
In Section \@ref(tidiness-modeling), we introduced the `tidy()` verb for statistical objects. There is also a `tidy()` method for recipes, discussed below. Before proceeding, let's create an extended recipe for the Ames data using some of the new steps show in the previous section: 
...
Our work here has used recipes solely inside of a workflow object. For modeling, that is the recommended use. However, for visualization and other activities, a workflow may not be appropriate; more recipe-specific functions may be required. Chapter \@ref(dimensionality) discusses lower-level APIs for fitting and using recipes. 
