...
***TODO*** fit does estimation for the recipe and model

```{r workflows-recipe-fit}
lm_fit <- fit(lm_wflow, ames_train)
```

***TODO*** what predict does
...
***TODO*** use extended recipe here for first time.

***TODO*** only use with pull

In Section \@ref(tidiness-modeling), we introduced the `tidy()` verb for statistical objects and in Section \@ref(recipes-manual), we showed how to `tidy()` a model. There is also a `tidy()` method for recipes, discussed below. Before proceeding, let's create an extended recipe for the Ames data using some of the new steps show in the previous section: 
...
***TODO*** note about extended use a recipes (outside of a workflow)
