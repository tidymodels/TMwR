...
#| fig.cap = "A bar chart of the frequencies of neighborhoods in the Ames training set.",
#| fig.alt = "A bar chart of the frequencies of neighborhoods in the Ames training set. The most houses reside in North Ames while the Greens, Green Hills, and Landmark neighborhood have very few instances."
...
recipe(~Bldg_Type, data = ames_train) %>% 
  ...
  arrange(`Raw Data`) %>% 
  kable(
    caption = "An illustration of binary encodings (i.e., 'dummy variables') for a qualitative predictor.",
    label = "dummy-vars"
  ) %>% 
...
bind_cols(ord_data, ord_contrasts) %>% 
  kable(
    caption = "An illustration of polynomial encodings for an ordered qualitative predictor.",
...
After exploring the Ames training set, we might find that the regression slopes for the gross living area differ for different building types. The following code is used to create Figure \@ref(fig:building-type-interactions):
...
#| echo = FALSE,
#| fig.cap = "Scatter plots of gross living area (in log-10 units) versus sale price (also in log-10 units) for five different building types.",
...
If you have ever used `geom_smooth()` within a `ggplot`, you have probably used a spline representation of the data. For example, function function below is used to create a visualization (Figure \@ref(ames-latitude-splines)) for different numbers of smooth splines for the latitude predictor:
...
#| echo = FALSE,
#| fig.cap = "Scatter plots of sale price versus latitude with trend lines using natural splines with different degrees of freedom.",
