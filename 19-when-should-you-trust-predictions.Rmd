...
grid_pred %>% 
    ...
    ggplot(aes(x = x, y = y)) + 
    geom_raster(aes(fill = `Probability of Class 1`), alpha = .5) +
...
map_dfr(seq(0, .1, length.out = 40), eq_zone_results) %>% 
  ...
  ggplot(aes(x = buffer, y = value, col = statistic, lty = statistic)) + 
...
As shown in Figure \@ref(fig:equivocal-zone-results), accuracy improves by a few percentage points but at the cost of nearly 10% of predictions being unusable! The value of such a compromise depends on how the model predictions will be used. 
...
res_test %>%
  mutate(day = add_day(.)) %>%
  ggplot(aes(x = date)) +
  geom_point(aes(y = ridership, pch = day), size = 3) +
...
test_dist_hist <- 
  ...
  geom_vline(xintercept = testing_pca$distance, col = "lightblue", lty = 2)  + 
