 ...
 * If a house did not have a particular feature, it was instinctively encoded as missing. For example, there were `r format(sum(ames$Alley == "No_Alley_Access"), big.mark = ",")` properties that did not have an alleyway. Instead of leave these as missing, they were relabeled to indicate that no alley was available.
 ...
 * Are there any odds or noticeable things about the distributions of the individual predictors? Is there much skewness or pathological distributions? 

 * Are there high correlations between predictors? For example, there are multiple predictors related to the size of the house. 

 * Are there associations between predictors and the outcomes? 

Finally, the data contain several columns related to the _quality_ of the house: 
...
These are mostly factors variables that are determined by the assessor's office when examining the property. As expected, many of these columns have strong relationships with sale price. However, are they _predictive_ of the outcome? They are impressionistic measurements of worth and, as such, would be more appropriately categorized as a separate set of _outcomes_ rather than predictors. This is a relatively subjective notion itself since one could argue that they are predictors. However, in subsequent analyses, these quality-related columns are _not_ included as model predictors. To do this:

```{r ames-subset}
ames <- 
  make_ames() %>% 
  # Remove columns that contain "_Qu"
