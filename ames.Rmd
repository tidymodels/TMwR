...
```{r ames-map, out.width = "100%", echo = FALSE, fig.cap = "Neighborhoods in Ames IA"}
# See file extras/ames_sf.R
res <- file.copy("premade/ames.png", "_book/premade/ames.png")
knitr::include_graphics("premade/ames.png")
```

We can see a few noticeable patterns. First, there is a void of data points in the center of Ames. This corresponds to Iowa State University. Second, while there are a number of neighborhoods that are geographically isolated, there are others that are adjacent to multiple neighborhoods. For example,  Timberland is located apart from almost all other neighborhoods:

```{r ames-timberland , out.width = "80%", echo = FALSE}
# See file extras/ames_sf.R
res <- file.copy("premade/timberland.png", "_book/premade/timberland.png")
knitr::include_graphics("premade/timberland.png")
```

Additionally, the Meadow Village neighborhood in Southwest Ames appears as an island of properties ensconced inside the sea of properties that make up the Mitchell neighborhood: 

```{r ames-mitchell , out.width = "70%", echo = FALSE}
# See file extras/ames_sf.R
...
```{r ames-northridge , out.width = "90%", echo = FALSE}
# See file extras/ames_sf.R
res <- file.copy("premade/northridge.png", "_book/premade/northridge.png")
knitr::include_graphics("premade/northridge.png")
```

Also, there are ten isolated houses labeled as being in Crawford but are not close to the majority of the other data in that neighborhood:

```{r ames-crawford , out.width = "80%", echo = FALSE}
# See file extras/ames_sf.R
...
```{r ames-dot_rr , out.width = "100%", echo = FALSE}
# See file extras/ames_sf.R
