...
If you would like the underlying modeling method to do what it would normally do with the data, `add_variables()` is a helpful interface. As will be seen in Section \@ref(special-model-formulas), it also facilitate more complex modeling specifications. However, as is also mentioned in the next section, models such as `glmnet` and `xgboost` expect the user to make indicator variables from factor predictors. In these cases, a recipe or formula interface might be a better choice. 
...
Workflow sets are mostly designed to work with resampling, which is discussed in Chapter \@ref(resampling). In the object above, the columns `option` and `result` must be populated with specific types of objects that result from resampling. This will be demonstrated in more detail in Chapter \@ref(compare).  
...
There's a lot more to workflow sets. Their main advantages won't be illustrated until later chapters. 
