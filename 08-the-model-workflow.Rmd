...
There is a third interface for passing data to the model. The `add_variables()` function using a simple `r pkg(dplyr)` syntax. The function has two primary arguments: `outcomes` and `predictors`. These use a selection approach similar to `r pkg(tidyr)` package to capture multiple selectors using `c()`. 
...
When the model is fit, the specification simply assembles these data, unaltered, into a data frame and passes it to the underlying function:

```{r workflows-add-variables-fit}
fit(lm_wflow, ames_train)
```

The downside to this method is that, for models like `xgboost`, any factor predictors are not converted into indicators variables. On the other hand, if you would like the underlying modeling method to do what it would normally do with the data, `add_variables()` is a helpful interface. As seen in the next section, it also facilitate more complex modeling specifications. 
...
## Creating workflows _en masse_  {#workflow-sets-intro}

There are some occasions where the data will require numerous attempts to find an appropriate model. For example: 

* For predictive models, it is advisable to evaluate  a variety of different model types. This will require the user to create multiple model specifications. 

* Sequential testing of models typically start with an expanded set of predictors. This "full model" is compared to a sequence of the same model that removes each predictor in turn. Using basic hypothesis testing methods or empirical validation, the effect of each predictor can be isolated and assessed. 

In these situations, as well as others, it might become tedious or onerous to create a lot of workflows from different sets or preprocessors and/or model specifications. To compensate for this problem, the `r pkg(workflowset)` package creates combinations of workflow ingredients. A list of preprocessors (e.g., formulas, recipes, or `r pkg(dplyr)` selectors) can be combined with a list of model specifications, resulting in a set of workflows. 
...
location_models$workflow[[1]]
```

Workflow sets are mostly designed to work with resampling, discussed in Chapter \@ref(resampling) and the columns `option` and `result` must be populated with specific types of objects that use resampling. This will be demonstrated in Chapter \@ref(compare).  

In the meantime, let's create model fits for each formula into a new column called `fit` using basic `r pkg(dplyr)` operations: 

```{r workflows-set-fit}
location_models <- 
  location_models %>% 
  mutate(fit = map(workflow, fit, ames_train))

location_models
location_models$fit[[1]]
```

There's a lot more to workflow sets and their main advantages won't be illustrated until later chapters. 
