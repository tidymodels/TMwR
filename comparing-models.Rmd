...
rsq_estimates <- 
  ...
  mutate(difference = `random forest` -  `linear regression`)
rsq_estimates
```
...
# or use an ANOVA model on the differences
lm(difference ~ 1, data = rsq_estimates) %>% 
  tidy(conf.int = TRUE)
```

what about 3+ models? 

what about real probability statements? 


## Bayesian methods

$$y_{ij} = (\beta_0 + b_{i}) + \beta_1x_{ij} + \epsilon_{ij}$$

$\beta_j \sim N(0, \sigma)$, and $b_i \sim t(1)$ 
