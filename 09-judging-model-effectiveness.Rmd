...
#| fig.cap = "Scatter plots of numeric observed versus predicted values for models that are optimized using the RMSE and the coefficient of determination.",
...
Let's plot the data before computing metrics using this code (results in Figure \@ref(fig:ames-performance-plot)):
...
#| echo = FALSE,
#| fig.cap = "Scatter plots of numeric observed versus predicted values for an Ames regression model. Both axes using log-10 units.",
#| fig.alt = "Scatter plots of numeric observed versus predicted values for an Ames regression model. Both axes using log-10 units. The model shows good concordance with some poorly fitting points at the high- and low-cost price scale."
```
...
The `two_class_curve` object can be used in a `ggplot` call to visualize the curve. There is an `autoplot()` method that will take care of the details. This code produces the plot in Figure \@ref(fig:example-roc-curve):
...
The results are in Figure \@ref(fig:grouped-roc-curves). 
